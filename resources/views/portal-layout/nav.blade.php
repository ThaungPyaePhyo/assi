...
<nav class="bg-gray-800 text-gray-50 border-gray-200 fixed w-full left-0 top-0 z-50 transistion duration-500 ease-in-out" id="navbar">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="" class="flex items-center">
            <img src="{{ asset('images/movie-ticket.png') }}" class="h-8 mr-3 text-white" alt="" />
            <span class="self-center text-2xl font-semibold whitespace-nowrap">My Cinema</span>
        </a>
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
            <divsky class="relative mt-3 md:hidden">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    <svg class="w-4 h-4 text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"/>
                    </svg>
            ...
            <x-nav.unorder-list>
                <x-nav.order-list title="Home" :route="route('home')"/>
                <x-nav.order-list title="Movies" :route="route('movie')"/>
                <x-nav.order-list title="About" :route="route('about.us')"/>
                <x-nav.order-list title="Contact Us" :route="route('contact.us')"/>
            </x-nav.unorder-list>
        </div>
    </div>
</nav>
...
<script>
    ...
    window.addEventListener('scroll', function() {
        ...
        if (scrollTop > lastScrollTop) {
            document.getElementById("navbar").style = "translate: 0 -100%";
        } else {
            document.getElementById("navbar").style = "translate: 0 0";
        }
